...
const REPO_BASE_URL =
  ...
  "https://raw.githubusercontent.com/your-org/dropkit/main";

const SITE_BASE_URL = process.env.SITE_BASE_URL || "https://dropkit.dev";
...
function validateManifest(manifest, type) {
    ...
    if (schema.properties) {
      for (const [key, def] of Object.entries(schema.properties)) {
        ...
        if (def.enum && !def.enum.includes(val)) {
          errors.push(`"${key}" must be one of [${def.enum.join(", ")}], got "${val}"`);
        }
      }
    }
  }
  ...
  if (type === "skill" || type === "agent") {
    ...
    if (manifest.id && !/^[a-z0-9][a-z0-9-]*[a-z0-9]$/.test(manifest.id)) {
      errors.push(`"id" must be lowercase alphanumeric with hyphens: "${manifest.id}"`);
    }
  ...
  if (type === "skill") {
    ...
    if (manifest.targets && !manifest.targets.default && !Object.keys(manifest.targets).some(k => k !== "default")) {
...
function loadTargets() {
  ...
  for (const file of fs.readdirSync(TARGETS_DIR)) {
    ...
    const data = JSON.parse(fs.readFileSync(path.join(TARGETS_DIR, file), "utf-8"));
    ...
    for (const [targetId, targetConf] of Object.entries(manifest.targets)) {
      ...
      if (typeof targetConf === "object" && targetConf.file) {
        ...
        if (!fs.existsSync(overridePath)) {
          warnings.push(`Override file missing for "${targetId}": ${targetConf.file}`);
        }
      }
    }
  }
...
function buildInstallUrls(manifest, relativePath, targets) {
  ...
  for (const targetId of Object.keys(targets)) {
    ...
    if (manifest.targets?.[targetId] || manifest.targets?.default) {
      install[targetId] = `curl -sL ${SITE_BASE_URL}/i/${targetId}/${manifest.id} | bash`;
    }
  }
...
function main() {
  ...
  const errors = [];
  const warnings = [];
  const stats = { skills: 0, agents: 0, collections: 0, errors: 0, warnings: 0 };
  ...
  for (const manifestPath of skillManifests) {
    ...
    try {
      ...
      errors.push({ path: relativePath, message: `Invalid JSON: ${e.message}` });
      stats.errors++;
      continue;
    }
    ...
    const fileWarnings = validateReferencedFiles(manifest, manifestDir, "skill");
    ...
    const supportedTargets = Object.keys(manifest.targets || {}).filter(k => k !== "default");
  ...
  for (const manifestPath of agentManifests) {
    ...
    const fileWarnings = validateReferencedFiles(manifest, manifestDir, "agent");
    for (const w of fileWarnings) {
      warnings.push({ path: relativePath, message: w });
      stats.warnings++;
    }
  ...
  if (fs.existsSync(COLLECTIONS_DIR)) {
    for (const file of fs.readdirSync(COLLECTIONS_DIR)) {
      ...
      try {
        ...
        errors.push({ path: `collections/${file}`, message: `Invalid JSON: ${e.message}` });
        stats.errors++;
        continue;
      }

      if (!collection.id || !collection.name) {
        errors.push({ path: `collections/${file}`, message: 'Missing "id" or "name"' });
        stats.errors++;
        continue;
      }
      ...
      for (const sid of collection.skills || []) {
        if (!skillIds.has(sid)) {
          warnings.push({ path: `collections/${file}`, message: `References unknown skill: "${sid}"` });
          stats.warnings++;
        }
      }
      ...
      for (const aid of collection.agents || []) {
        if (!agentIds.has(aid)) {
          warnings.push({ path: `collections/${file}`, message: `References unknown agent: "${aid}"` });
          stats.warnings++;
        }
      }
